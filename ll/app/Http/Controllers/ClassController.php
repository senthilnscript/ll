...
class ClassController extends Controller {
	...
	public function store()
	{
		...
		{
			$class=DB::table('class')->where('name', $_POST['name'])->get();
			if(empty($class))
			{
				$cl=DB::table('class')->insert(['name' => $_POST['name'], 'description' => $_POST['description'],'created_at'=>date("Y-m-d H:i:s"),'updated_at'=>date("Y-m-d H:i:s")]);
	...
	public function update($id)
	{
		...
		{
			$contact=DB::table('class')->where('id', $id)->get();
			if(isset($contact[0])&&!empty($contact[0]))
			{
				$class=DB::table('class')->where('name', $dat['name']) ->whereNotIn('id', [$id])->get();
				if(empty($class))
				{
					$cl=DB::table('class')->where('id', $id)->update(['name' => $dat['name'], 'description' => $dat['description'],'updated_at'=>date("Y-m-d H:i:s")]);
					$result=array("status"=>true,"message"=>"success");
				}
				else
				{
	...
	{
		$contact=DB::table('class')->where('id', $id)->first();
	...
	{
		$contact=DB::table('class')->where('id', $id)->delete();;
	}
